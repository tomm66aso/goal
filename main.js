...
/* ══ LANG PREFERENCE — prima visita sempre IT ══ */
(function(){
  var KEY = 'dg_lang';
  var isEN = location.pathname.indexOf('/en') === 0 || location.pathname.indexOf('/en/') > -1;
  if (isEN) {
    localStorage.setItem(KEY, 'en');
  } else {
    if (!localStorage.getItem(KEY)) localStorage.setItem(KEY, 'it');
  }
  document.querySelectorAll('a[hreflang="en"]').forEach(function(a){
    a.addEventListener('click', function(){ localStorage.setItem(KEY, 'en'); });
  });
  document.querySelectorAll('a[hreflang="it"]').forEach(function(a){
    a.addEventListener('click', function(){ localStorage.setItem(KEY, 'it'); });
  });
})();
